...
def parse_races(input_of_day: str) -> List[PastRace]:
    time_str: str = input_of_day.splitlines()[0].strip()
    distance_str: str = input_of_day.splitlines()[1].strip()

    times = [int(time_str[time.start():time.end()]) for time in re.finditer(r"[\d]+", time_str)]
    distances = [int(distance_str[distance.start():distance.end()]) for distance in re.finditer(r"[\d]+", distance_str)]

    for i in range(len(times)):
        yield PastRace(times[i], distances[i])
...
if __name__=='__main__':
    ...
    inputOfDay = ''
    with open('../input/day06-1.input', 'r') as f:
        inputOfDay = f.read()
    #test()
